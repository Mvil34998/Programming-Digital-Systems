...
Назначение диаграммы: показать основные слои базы знаний от масштаба проекта до будущих книг.
...
Назначение диаграммы: показать подготовку базы знаний к серии книг.

```mermaid
flowchart TD
    A[Книжный слой] --> B[Book 01 Foundations]
    A --> C[Book 02 System Design]
    A --> D[Book 03 System Architecture Design]
    A --> E[Book 04 Technical Requirements]
    A --> F[Book 05 Toolchain Selection]
    A --> G[Book 06 Implementation Architecture]
    A --> H[Book 07 Testing]
    A --> I[Book 08 Operation Maintenance Evolution]

    B --> J[Фундамент]
    C --> K[Проектирование системы]
    D --> L[Архитектура системы]
    E --> M[Требования]
    F --> N[Инструментарий]
    G --> O[Реализация]
    H --> P[Проверка]
...
## 11. Правила использования диаграмм из документа
...
- Initial version: созданы диаграммы структуры документации, слоёв базы знаний, roadmap, анкет, энциклопедии, примеров, диаграмм и книжного слоя.
