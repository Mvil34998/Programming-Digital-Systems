...
- `docs/00_maps/Development_Route_Map.md`
  - Передаёт: маршрут движения от идеи к реализации.
...
- События:
  ...
  - отчёт создан.
- Потоки:
  - файл → чтение → проверка → расчёт → отчёт.
- Хранение:
...
- События:
  - поле изменено;
  - сохранение запрошено;
  - экспорт запрошен.
- Потоки:
  - действие пользователя → обновление модели → предпросмотр → экспорт.
- Хранение:
  - шаблоны;
...
- Состояния:
  ...
  - fault.
- События:
  - значение датчика изменилось;
  - таймер истёк;
  - ошибка обнаружена.
- Потоки:
  - датчик → фильтрация → решение → команда.
- Хранение:
  - конфигурация;
...
- События:
  ...
  - авария подтверждена.
- Потоки:
  - входной сигнал → межблокировка → команда выходу.
- Хранение:
  - уставки;
  - аварийный журнал.
- Ошибки:
  - потерян safety-сигнал;
  - активна межблокировка.
...
- Состояния:
  - файл загружен;
  - инструмент найден;
  - операция анализируется;
  - отчёт создан.
- События:
  - NC-файл загружен;
  - вызов инструмента найден;
  - ошибка парсинга обнаружена.
- Потоки:
  - NC-файл → парсинг → анализ → расчёт → отчёт.
- Хранение:
  - история использования инструмента;
  - архив отчётов.
- Ошибки:
  - неизвестный формат NC-программы;
...
- входные данные для `Roadmap_System_Architecture_Design.md`.
