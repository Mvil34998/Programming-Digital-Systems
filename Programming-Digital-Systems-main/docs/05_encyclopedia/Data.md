...
> [!info] Главное
> Данные — базовый элемент проектирования цифровой системы.
> Если данные определены плохо, система начинает принимать, хранить и передавать значения без понятного смысла, проверки и связи с сущностями.

## 2. Место документа в системе знаний

Документ относится к энциклопедическому слою проекта Programming Digital Systems.

Документ используется после [[docs/05_encyclopedia/Entities|Entities]].
...
> [!tip] Простая формула
> Если система что-то получает, проверяет, хранит, передаёт или формирует как результат — это нужно описать как данные.
...
> [!warning] Не путать
> Данные — это не просто поля или переменные. Данные должны иметь источник, назначение, формат, правило проверки и место использования.
...
> [!important] Правило
> Данные нельзя проектировать отдельно от сущностей, правил, хранения, интерфейсов и ошибок.
...
> Практический анализ данных начинается с вопроса: какие значения система получает, какие создаёт, какие сохраняет и какие отдаёт наружу?
...
После работы с данными необходимо перейти к [[docs/05_encyclopedia/Rules|Rules]] и определить правила проверки, обработки, хранения и передачи данных.
...
Этот раздел переводит понятие данных в рабочий элемент будущей метамодели Digital System CAD.

### 15.1. Definition

В метамодели Digital System CAD данные — это типизированные значения, поля, структуры, сообщения или наборы значений, которые описывают сущности, состояния, события, правила, потоки, интерфейсы, хранение или результаты работы системы.

Для важных данных нужно фиксировать:
...
В Digital System CAD данные не должны описываться как случайные поля или переменные. Данные должны быть привязаны к владельцу: сущности, событию, интерфейсу, состоянию, потоку, хранилищу или результату.

Данные являются материалом для правил, проверок, форм, таблиц, SDD-разделов, тестовых сценариев и генераторов.

### 15.3. Not examples

Данными не следует считать:

- сущность целиком;
- правило проверки;
- действие пользователя;
- модуль реализации;
- формат хранения без описания смысла;
- UI-элемент без указания, какие данные он показывает или меняет.

Если значение не имеет владельца, источника и назначения, его нужно пометить как кандидат и уточнить.

### 15.4. Related model elements

Данные должны быть связаны с:

- `Entity` — владелец или предмет описания;
- `Rule` — проверка, ограничение или расчёт;
- `Event` — данные события;
- `Flow` — маршрут передачи данных;
- `Storage` — место сохранения;
- `Interface` — место ввода, вывода или обмена;
- `Error` — реакция на некорректные данные;
- `Requirement` — требование к данным;
- `TestCase` — проверка формата, диапазона, обязательности или реакции.
...
- `DataField has Format`;
- `DataField has AllowedValueSet`;
- `Rule validates DataField`;
- `Flow transfers DataField`;
- `Storage stores DataField`;
- `Interface displays DataField`;
- `Interface accepts DataField`;
- `Error caused_by invalid DataField`;
- `TestCase verifies DataField rule`.
...
Данные считаются достаточно описанными для текущего этапа, если можно ответить:

1. Есть ли у данных `id` и понятное имя?
2. Указан ли владелец данных?
3. Понятен ли источник данных?
4. Понятно ли назначение данных?
5. Указан ли формат и тип?
6. Указана ли обязательность?
7. Указаны ли допустимые значения или диапазоны?
8. Есть ли правило проверки?
9. Определено ли действие при ошибке?
10. Понятно ли, где данные используются, передаются или хранятся?

### 15.8. Open questions

Для будущей метамодели нужно уточнить:

- как различать `Data`, `DataField`, `Parameter`, `Signal`, `Message` и `Document`;
- какие поля обязательны для простого значения, структуры, файла и сообщения;
- как описывать единицы измерения;
- как фиксировать производные и расчётные данные;
- как хранить историю изменения критичных данных.
...
- Updated: добавлена интерпретация для Digital System CAD: данные описаны как типизированные элементы модели с владельцем, правилами, ошибками, потоками и проверками полноты.
