...
`Entities.md` раскрывает понятие сущности как одного из базовых элементов проектирования цифровых систем.
...
## 28. Критерии завершения работы с сущностями
...
После работы с сущностями необходимо перейти к [[docs/05_encyclopedia/Data|Data]] и определить данные, атрибуты и информационные представления для каждой важной сущности.
...
## 31. Интерпретация для Digital System CAD

Этот раздел переводит понятие сущности из энциклопедического объяснения в рабочий элемент будущей метамодели Digital System CAD.

### 31.1. Definition

В метамодели Digital System CAD сущность — это типизированный элемент модели, который имеет устойчивую идентичность, определение, назначение, контекст применения и связи с другими элементами цифровой системы.

Сущность не должна существовать только как имя. Для важной сущности нужно фиксировать:
...
Сущность используется как опорная точка для описания данных, правил, состояний, событий, потоков, хранения, интерфейсов, ошибок, требований, тестов и архитектурных решений.

В Digital System CAD сущность должна рассматриваться не как будущий класс в коде, а как смысловой объект модели, который может иметь разные представления в документах, диаграммах, таблицах, SDD, тестах и реализации.

### 31.3. Not examples

Сущностью не следует считать:

- случайную переменную;
- отдельное поле данных без собственной идентичности;
- временное значение расчёта;
- действие или команду;
- UI-надпись;
- технологию хранения;
- файл кода;
- диаграммный блок без определения и связей.

Если объект похож на сущность, но его границы неясны, нужно зафиксировать открытый вопрос, а не угадывать.

### 31.4. Related model elements

Сущность должна быть связана с другими элементами модели:

- `DataField` — данные, которые описывают сущность;
- `Rule` — ограничения и проверки, действующие на сущность;
- `State` — допустимые положения сущности;
- `Event` — факты, которые происходят с сущностью;
- `Flow` — сценарии, в которых сущность участвует;
- `Storage` — место, где сущность или её данные сохраняются;
- `Interface` — способ взаимодействия пользователя или внешней системы с сущностью;
- `Error` — нарушения, связанные с сущностью;
- `Requirement` — требование, в котором сущность участвует;
- `TestCase` — проверка поведения или правил сущности.

### 31.5. Related relations

Типовые связи для сущности:
...
- `Requirement references Entity`;
- `TestCase verifies Entity behavior`;
- `Module implements Entity representation`.

Связь должна рассматриваться как отдельный факт модели, а не как пояснение в свободном тексте.

### 31.6. Structured facts
...
Такая форма нужна, чтобы в дальнейшем строить таблицы, диаграммы, SDD-разделы, проверки полноты и контекст для Codex из одной модели.

### 31.7. Validation questions

Сущность считается достаточно описанной для текущего этапа, если можно ответить:

1. Есть ли у сущности устойчивый `id`?
2. Есть ли определение, а не только название?
3. Понятно ли назначение сущности?
4. Понятен ли контекст, где сущность существует?
5. Известен ли источник знания о сущности?
6. Указаны ли данные сущности?
7. Указаны ли правила, состояния, события и потоки?
8. Понятно ли, нужно ли сущность хранить?
9. Известны ли интерфейсы, через которые сущность видна или изменяется?
10. Зафиксированы ли открытые вопросы?

### 31.8. Open questions

Для будущей метамодели нужно отдельно уточнить:

- где проходит граница между `Entity`, `DataField`, `ValueObject`, `DTO`, `Record` и `ViewModel`;
- когда информационный объект становится сущностью;
- как различать предметные сущности и системные сущности;
- какие минимальные поля обязательны для разных типов сущностей;
- какие типы связей должны быть в контролируемом словаре.
...
- Updated: добавлена интерпретация для Digital System CAD: сущность описана как типизированный элемент модели со связями, структурированными фактами, проверками полноты и открытыми вопросами.
